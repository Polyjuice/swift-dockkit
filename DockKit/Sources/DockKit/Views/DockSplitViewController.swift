...
public class DockSplitViewController: NSSplitViewController {
    ...
    /// The panel this controller represents (must have content == .group with style == .split)
    public private(set) var panel: Panel

    /// Map of child view controllers to their panel IDs
    ...
    /// The panel group (convenience accessor)
    private var group: PanelGroup {
        get {
            guard case .group(let g) = panel.content else {
                fatalError("DockSplitViewController requires a panel with .group content")
            }
            return g
        }
        set {
    ...
    public init(panel: Panel) {
        assert(panel.group?.style == .split, "DockSplitViewController requires a panel with split group style")
        self.panel = panel
    ...
    public required init?(coder: NSCoder) {
        self.panel = Panel(
            content: .group(PanelGroup(style: .split))
        )
    ...
    public override func loadView() {
        ...
        customSplitView.isVertical = group.axis == .horizontal
    ...
    private func setupSplitView() {
        splitView.isVertical = group.axis == .horizontal
    ...
    /// Create appropriate view controller for a child panel
    private func createViewController(for childPanel: Panel) -> NSViewController {
        switch childPanel.content {
        case .group(let childGroup):
            switch childGroup.style {
            case .split:
                let splitVC = DockSplitViewController(panel: childPanel)
        ...
        case .content:
            // Leaf content panels should not appear as direct children of a split;
            // they are managed by their parent tab group. Create a minimal wrapper.
    ...
    /// Get the view controller for a specific panel ID
    public func childViewController(for nodeId: UUID) -> NSViewController? {
        return childNodeMap[nodeId]
    }

    /// Insert a new child panel at the specified index
    public func insertChild(_ childPanel: Panel, at index: Int) {
        let insertIndex = max(0, min(index, group.children.count))

        // Update model
        var g = group
        g.children.insert(childPanel, at: insertIndex)
        let count = g.children.count
        g.proportions = Array(repeating: 1.0 / CGFloat(count), count: count)
        self.group = g
    ...
    public func removeDockChild(at index: Int) {
        ...
        let panelId = group.children[index].id

        // Update model
        var g = group
        g.children.remove(at: index)
        let count = max(1, g.children.count)
        g.proportions = Array(repeating: 1.0 / CGFloat(count), count: count)
        self.group = g
        ...
        childNodeMap.removeValue(forKey: panelId)
    ...
    /// Replace a child panel with a new panel
    ...
    public func splitChild(at index: Int, direction: DockSplitDirection, withNewPanel newPanel: Panel) {
        guard index >= 0 && index < group.children.count else { return }

        let existingPanel = group.children[index]

        // Determine new split axis
        let newAxis: SplitAxis = (direction == .left || direction == .right) ? .horizontal : .vertical

        // Create new split panel containing existing and new
        let insertFirst = (direction == .left || direction == .top)
        let children = insertFirst ? [newPanel, existingPanel] : [existingPanel, newPanel]

        let newSplitPanel = Panel(
        ...
        replaceChild(at: index, with: newSplitPanel)
    }
...
extension DockSplitViewController {
    ...
    private func updateProportionsFromSplitView() {
        ...
        for item in splitViewItems {
            let itemSize = group.axis == .horizontal ?
